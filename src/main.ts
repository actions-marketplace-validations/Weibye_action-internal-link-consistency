...
import { CrossReference } from './CrossReferenceValidate';
import { GetExamplesFromDisk } from './DiskExamples';
import { FileData, ValidatedFileData } from './FileData';
...
// Config
const sourcePath: string = '__tests__/testData/examples/';
...
const pathToReadme: string = sourcePath + 'README.md';
const pathToCargo: string = 'Cargo.toml';
...
async function run(): Promise<void> {
    try {
        ...
        console.log('======= CARGO =======');
        const cargoExamples: FileData[] = checkCargo ? GetExamplesFromCargo(targetsPaths[0], filesToExclude, foldersToExclude) : new Array();
        ...
        console.log('======= README =======');
        let readmeExamples: FileData[] = checkReadme ? GetExamplesFromReadme(targetsPaths[1], filesToExclude, foldersToExclude) : new Array();
